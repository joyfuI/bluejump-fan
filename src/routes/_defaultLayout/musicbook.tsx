...
import { Button, Flex, Input, Menu, Space, Table } from 'antd';
import type {
  FilterDropdownProps,
  FilterValue,
  SorterResult,
...
import MusicbookRandomModal from '@/components/MusicbookRandomModal.tsx';
...
const RouteComponent = () => {
  ...
  const currentDataSource = useRef<DataType[]>([]);
  ...
  useEffect(() => {
    if (!isLoading) {
      currentDataSource.current = data;
    }
  }, [isLoading, data]);
  ...
  return (
    <>
      ...
      <Space className="my-4">
        <Button
          disabled={isLoading || currentDataSource.current.length === 0}
          icon={<Dices />}
      ...
      <MusicbookRandomModal
        data={currentDataSource.current}
        onClose={handleClose}
        open={open}
      />
    </>

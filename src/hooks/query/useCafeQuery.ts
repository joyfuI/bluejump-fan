...
const useCafeQuery = () => {
  return useQueries({
    queries: MEMBERS.flatMap(
      (member) =>
        member.cafeMenuIds?.map((cafeMenuId) => ({
          queryKey: ['getCafeArticles', member.cafeId, cafeMenuId],
          queryFn: () => getCafeArticles(member.cafeId, cafeMenuId),
